...
// Read rules.json for metadata — this is the single source of truth for template files
const rulesMetadata = JSON.parse(
  readFileSync(join(TEMPLATES_DIR, "rules.json"), "utf-8"),
);

// Derive template file list from rules.json instead of a hardcoded array
const templateFiles = rulesMetadata.rules.map((rule) => rule.filename);
